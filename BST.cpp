...
BSTNode::BSTNode()
{
	...
	left_ = nullptr;
	right_ = nullptr;
	//Declare initial height to be zero
	height = 0;
}
...
std::string BST::JSON() const {
	...
	if (root_ != nullptr) {
		...
		while (!nodes.empty()) {
			...
			//added balance factor to output json

			if(v->right_ == nullptr && v->left_ == nullptr)
			{
				balanceFactor = 0;
			}else if(v->right_ == nullptr)
			{
				balanceFactor = -1 - v->left_->height;
			}else if(v->left_ == nullptr)
			{
				balanceFactor = v->right_->height + 1;
			}else
			{
				balanceFactor = v->right_->height - v->left_->height;
			}
			
			result[key]["balance factor"] = balanceFactor;


			//addded node height to output json
